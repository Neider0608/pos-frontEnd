...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, CardModule, ButtonModule, InputTextModule, DropdownModule, DialogModule, InputSwitchModule, ProgressBarModule, TableModule, BadgeModule],
...
export class LowStockComponent implements OnInit {
    ...
    get filteredItems(): Stock[] {
        return this.lowStockItems.filter((item) => {
            ...
            const matchesSearch = item.name.toLowerCase().includes(this.searchTerm.toLowerCase()) || item.barcode.includes(this.searchTerm);
    ...
    getStatusBadge(status: string) {
        switch (status) {
            case 'critical':
                return { label: 'Crítico', tailwindClass: 'bg-rose-500/20 text-rose-700 border-rose-200' };
            case 'low':
                return { label: 'Bajo', tailwindClass: 'bg-orange-500/20 text-orange-700 border-orange-200' };
            default:
