...
export class WhatsappService {
    ...
    getConversations(phoneNumberId?: string): Observable<ApiResponse<Conversation[]>> {
        return this.http.get<ApiResponse<Conversation[]>>(`${environment.apiUrl}WhatsApp/GetConversations/${phoneNumberId}`);
    }
    ...
    saveAgent(agenteId: number, configId: number) {
        return this.http.post(`${environment.apiUrl}WhatsApp/SaveAgent`, {
    ...
    getAgents(configId: number) {
        return this.http.get<any[]>(`${environment.apiUrl}WhatsApp/GetAgents/${configId}`);
    }

    getAgentsByPhoneNumberId(phoneNumberId: string) {
        return this.http.get<any[]>(`${environment.apiUrl}WhatsApp/GetAgentsByPhoneNumberId/${phoneNumberId}`);
    }

    deleteAgent(id: number) {
        return this.http.delete(`${environment.apiUrl}WhatsApp/DeleteAgent/${id}`);
    }

    assignAgent(payload: any) {
        console.log('Assigning agent with payload:', payload);
        return this.http.post(`${environment.apiUrl}WhatsApp/AssignAgent`, payload);
    }
}
